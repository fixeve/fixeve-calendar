...
function getTitle(){
  ...
  if(currentView==="month") return `${y}年 ${m}月`;
  if(currentView==="week"){
...
function renderMonthView(){
  ...
  renderWeekDays();
  for(let i=0;i<first.getDay();i++) createEmptyDay();
...
function createEmptyDay(){const div=document.createElement("div");div.classList.add("day");daysContainer.appendChild(div);}
...
function dateKeyString(date){return `${date.getFullYear()}-${date.getMonth()+1}-${date.getDate()}`;}
function isToday(date){const now=new Date();return date.getFullYear()===now.getFullYear()&&date.getMonth()===now.getMonth()&&date.getDate()===now.getDate();}
function getWeekStart(date){const d=new Date(date);d.setDate(d.getDate()-d.getDay());return d;}
function getWeekEnd(date){const d=new Date(getWeekStart(date));d.setDate(d.getDate()+6);return d;}

function openModal(dateKey,date){modal.style.display="flex";modalDate.textContent=`${date.getMonth()+1}月${date.getDate()}日`;eventText.value="";renderEventList(dateKey);saveEventBtn.onclick=()=>saveEvent(dateKey);}
...
viewSelect.onchange=e=>{currentView=e.target.value;renderCalendar();}
renderCalendar();
